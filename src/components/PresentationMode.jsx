...
export default function PresentationMode({ slides, onClose }) {
  ...
  const fetchGeminiTTS = async (text) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key=${import.meta.env.VITE_GEMINI_API_KEY}`, {
        method: "POST",
