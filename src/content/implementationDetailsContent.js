export const implementationDetailMeta = {
  ...
  subtitle: 'فهرست تصمیم‌ها، دارایی‌ها، قراردادها، عملیات، شکست‌ها و آزمون‌هایی که باید پیش از ساخت هر بخش صریح شوند.',
  version: 'Detail Registry v1.1 · 2026-08-06',
}
...
export const implementationAreas = [
  ...
  area('telegram-ingress', 'telegram', 'Webhook، Update و Flood Control', 'ورودی تمام ربات‌ها از Gateway مشترک عبور می‌کند اما ترتیب، تکرار، محدودیت و خطای هر Bot جدا کنترل می‌شود.', {
    decisions: ['Webhook path غیرقابل حدس و secret header', 'کلید dedup و مدت نگهداری Update', 'partition queue بر اساس bot/chat/user', 'ترتیب لازم برای هر نوع Update', 'سقف نرخ هر Bot و هر chat'],
    inventory: ['public webhook URL', 'allowed_updates', 'max_connections', 'queue/topic', 'dead-letter queue', 'rate-limit policy'],
    contracts: ['TelegramUpdateEnvelope', 'InboundMessageReceived', 'UpdateProcessed/Failed', 'RetrySchedule', 'OutboundMessageCommand'],
    operations: ['ACK سریع و پردازش async', 'deduplicate قبل از side effect', 'backoff با jitter', '429 retry_after handling', 'replay از DLQ با audit'],
    failures: ['Update تکراری', 'Update خارج از ترتیب', 'webhook spoofing', 'flood/429', 'صف عقب‌مانده', 'پاسخ پس از حذف پیام/chat'],
    tests: ['replay update یکسان', 'burst و flood control', 'out-of-order scenario', 'invalid secret rejection', 'queue poison message'],
  ...
  area('mini-app', 'telegram', 'Mini App، دامنه و اعتبارسنجی initData', 'Mini App فقط پوسته است و اعتماد به داده تلگرام پس از اعتبارسنجی سرور، کنترل عمر و جلوگیری از Replay شکل می‌گیرد.', {
    decisions: ['دامنه مجاز هر Bot/Environment', 'حداکثر عمر initData', 'session exchange و TTL', 'deep-link/start_param schema', 'قابلیت‌های client و fallback چت'],
    inventory: ['Mini App URL', 'BotFather menu/web_app config', 'CSP و allowed origins', 'session signing key', 'theme/locale config'],
    contracts: ['InitDataExchange', 'TelegramIdentityLink', 'StartParameter', 'MiniAppSession', 'DeepLinkTarget'],
    operations: ['اعتبارسنجی hash روی سرور', 'nonce/replay cache', 'session revoke', 'domain rotation', 'compatibility telemetry'],
    failures: ['اعتماد به user object سمت client', 'initData منقضی یا replay', 'دامنه اشتباه محیط', 'webview قدیمی', 'back button/close بدون ذخیره'],
    tests: ['tampered initData', 'expired auth_date', 'replay nonce', 'RTL/theme/device matrix', 'deep-link از همه Botها'],
